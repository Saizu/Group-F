...
namespace Complete{
    public class TankMovement : MonoBehaviour{
        ...
        public int ammoStock = 0;  // ストック数
        private float m_TurretTurnInputValue;
        public float m_TurretTurnSpeed = 90f;
       ...
       private void Move (){
        ...
        private void OnTriggerEnter(Collider other){  //弾薬すり抜け
          if (other.CompareTag("Ammo")){
             TankShooting tankShooting = GetComponent<TankShooting>();
             if (tankShooting != null){
                tankShooting.CurrentAmmo += 10;
                tankShooting.UpdateAmmoUI();
                Debug.Log("残弾数 : " + tankShooting.CurrentAmmo);
             }
           Destroy(other.gameObject); //弾薬は1度のみ有効
         }
        }
  }
}
